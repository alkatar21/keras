    ...
    def add_endpoint(self, name, fn, input_signature=None):
        ...
        if isinstance(fn, tf.types.experimental.GenericFunction):
            if not fn._list_all_concrete_functions():
                raise ValueError(
                    f"The provided tf.function '{fn}' has never been called. "
                    "To specify the expected shape and dtype of the function's "
                    "arguments, you must either provide a function that "
