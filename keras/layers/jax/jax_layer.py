...
class JaxLayer(Layer):
    ...
    - `inputs`: inputs to the model, a JAX `DeviceArray` or a `PyTree` of
      `DeviceArray`s.
    ...
    - `inputs`: a JAX `DeviceArray` or a `PyTree` of `DeviceArray`s with dummy
    ...
    - `apply_fn` must return a `tuple` containing the outputs of the model and
